...
void STEPPER_init (t_stepper *f)
{
	f->name = 0;
	f->type = 
}

void STEPPER_getValue(t_stepper *f, int step, double start, double stop)
{
	switch (f->type) {
		case:
...
t_stepper *STEPPER_new (int nParameters, const char *identifier)
{
	t_DMDshape *f;
	int id;
	int i;
...
void DMDSHAPE_rotateScaleAndMoveCanvas(int panel, int ctrl, double rotationAngle, double scale, int offsetx, int offsety, t_waveform* dmdWaveform)
{
	...
	char bitmapFilename[MAX_PATHNAME_LEN];
	char bitmapFilename2[MAX_PATHNAME_LEN]; 
	...
	if (strlen(config->pathMagickConvert) == 0) {
	...
	if (dmdWaveform != NULL) {
		...
		if (0 && DMDimage != 0 && DMDimage->enableDMDPhaseFeedback) { // not active
			...
			offsetx = offsetx + pixelsPerSiteX*phaseX; // TODO FIXME not tested
			offsety = offsety + pixelsPerSiteY*phaseY; // TODO FIXME not tested  
		}
	} else {
	...
	sprintf (bitmapFilename, "%s\\dmd_image_tmp.png", config->autoSaveSequencesPath);
	...
	sprintf(batchCommand,"\"%s\" %s -filter Point +matte -virtual-pixel white -distort SRT \"%d,%d  %.2f  %.2f  %d,%d\" +repage %s",config->pathMagickConvert,bitmapFilename,width/2,height/2,scale,rotationAngle,width/2+offsetx,height/2+offsety,bitmapFilename2);
...
void DMDSHAPE_drawRing (double *p, int panel, int ctrl, t_waveform *dmdWaveform)
{
	int width,height;
...
void DMDSHAPE_addRing (void) 
{
	...
	f->plotFunc = DMDSHAPE_drawRing; //(DMDshape_plotFunctPtr)
}


void DMDSHAPE_drawBox (double *p, int panel, int ctrl, t_waveform *dmdWaveform)
{
	int width,height;
...
void DMDSHAPE_addBox (void) 
{
	...
	f->plotFunc = DMDSHAPE_drawBox; //(DMDshape_plotFunctPtr)
}

void DMDSHAPE_drawSimpleGrid (double *p, int panel, int ctrl, t_waveform *dmdWaveform)
{
	int width,height;
	int x,y;
...
void DMDSHAPE_addSimpleGrid (void) 
{
	...
	f->plotFunc = DMDSHAPE_drawSimpleGrid;   //(DMDshape_plotFunctPtr)
}	


 
void DMDSHAPE_drawMPQ (double *p, int panel, int ctrl, t_waveform *dmdWaveform)
{
	int width,height;
...
void DMDSHAPE_addMPQ (void) 
{
	...
	f->plotFunc = DMDSHAPE_drawMPQ; //(DMDshape_plotFunctPtr)
}



void DMDSHAPE_singleSiteAddressing (double *p, int panel, int ctrl, t_waveform *dmdWaveform)
{
	int width,height;
	int picWidth,picHeight;
	char dirname[MAX_PATHNAME_LEN]; 
	...
	double offsetx, offsety, dx, dy,phaseOffsetX, phaseOffsetY;
	int x, y;
	int xi,yi;
	...
	double lattYangle;
	double s;
	double phaseX,phaseY,tmp,pixelsPerSiteX,pixelsPerSiteY;
	...
	if (lattXangle == lattYangle) {
		tprintf("Error during dmd single site calculation. Lattice angles are not allowed to be the same.\n");
		return;	
	}
...
//	if (x > WFM_ADDR_MATRIXDIM) return 0;
	...
	if (DMDimage->enableDMDPhaseFeedback) {
		...
		phaseOffsetX = pixelsPerSiteX*phaseX; // TODO FIXME not tested
		phaseOffsetY = pixelsPerSiteY*phaseY; // TODO FIXME not tested  
	}
	...
	for (x = 0; x < WFM_ADDR_MATRIXDIM; x++) {
		for (y = 0; y < WFM_ADDR_MATRIXDIM; y++) {  
			matrixVal = WFM_ADDR_getMatrixValue (dmdWaveform, x, y);
			if (matrixVal == 1) {
...
void DMDSHAPE_addSingleSiteAddressing (void) 
{
	...
	f->plotFunc = DMDSHAPE_singleSiteAddressing; //(DMDshape_plotFunctPtr)
}
...
t_DMDshape *DMDSHAPE_ptr (int nr)
{
	...
	if ((nr <= 0) || (nr > ListNumItems (DMDshapeList))) return NULL;

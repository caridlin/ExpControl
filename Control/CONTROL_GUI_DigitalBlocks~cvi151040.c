...
//					  VAL_DK_CYAN);
	...
	SetCtrlAttribute (panelSequence, SEQUENCE_TABLE_header2,
					  ATTR_TOP, 0);
	SetCtrlAttribute (panelSequence, SEQUENCE_TABLE_header2,
					  ATTR_WIDTH, panelWidth (panelSequence) - config->splitterVPos - VAL_LARGE_SCROLL_BARS);
	...
	SetCtrlAttribute (panelSequence, SEQUENCE_TABLE_dac2,
					  ATTR_TOP, ctrlBottom (panelSequence, SEQUENCE_TABLE_header1));
	SetCtrlAttribute (panelSequence, SEQUENCE_TABLE_dac2,
					  ATTR_WIDTH, panelWidth (panelSequence) - config->splitterVPos);
	...
	SetCtrlAttribute (panelSequence, SEQUENCE_TABLE_digital2, ATTR_WIDTH,
					  panelWidth (panelSequence) -config->splitterVPos);  
	...
	SetCtrlAttribute (panelSequence, SEQUENCE_TABLE_digital2, ATTR_HEIGHT, 
		panelHeight (panelSequence) - ctrlTop (panelSequence, SEQUENCE_TABLE_digital2));
	SetCtrlAttribute (panelSequence, SEQUENCE_TABLE_digital1, ATTR_HEIGHT, 
		ctrlHeight(panelSequence, SEQUENCE_TABLE_digital2) - VAL_LARGE_SCROLL_BARS);
...
void DIGITALBLOCKS_insertGpibCommandsToMenu (t_sequence *seq, int gpibCommandNr)
{
	...
	for (i = 1; i <= ListNumItems (seq->lGpibCommands); i++) {
		...
		if (g->transmitAfterStartOfSequence) {
...
void DIGITALBLOCKS_insertWaveformsToMenu (t_sequence *seq, int channel)
{
	...
	DiscardSubMenu (menuDacProperties, subMenu);
...
int DIGITALBLOCKS_createBitmapForChannel (t_sequence *seq, t_digitalBlock *start, int ch, int panel, int ctrl, int analogCh)
{
	...
	for (i = start->blockNr; i < start->blockNr+start->nElementsInGroup; i++) {
		...
		if (state) {
			...
			CanvasDrawRect (panel, ctrl,
							MakeRect (0, xLeft, config->buttonHeight, xWidth+1),
							VAL_DRAW_FRAME_AND_INTERIOR);
		}
...
void DIGITALBLOCKS_createBitmaps (t_sequence *seq, t_digitalBlock *b, int DACchannel, int DIOchannel)
{
	...
	if ((b->groupNr > 0) && (!b->isGroupExpanded) && (b->nElementsInGroup > 0)) {
		...
		SetCtrlAttribute (panelSequence, canvas, ATTR_HEIGHT,
						  config->buttonHeight);
		SetCtrlAttribute (panelSequence, canvas, ATTR_WIDTH,
						  b->tableColumnWidth);
//		ProcessDrawEvents();
		if (DACchannel >= 0) {
...
void DIGITALBLOCKS_displayDACChannelInTable (int panel, int ctrlDac, t_sequence *seq, t_digitalBlock *b, int i)
{
	...
	dimmed =  (b->waveforms[i] == WFM_ID_UNCHANGED) || b->disable;
...
//	bgcolor = WFM_hasVariableParameters(wfm) ? VAL_ORANGE : colors[i % NColors];
	...
	switch (b->waveforms[i]) {
		...
		case WFM_ID_CONSTVOLTAGE:
			...
			break;
		default:
			offColor = (b->groupNr > 0) ? VAL_OFFWHITE :  TABLE_COLOR_BUTTON_INACTIVE_DAC[(i+1)%2];
...
//					else if (seq->DIO_lockChannel[ch]) color = TABLE_COLOR_BUTTON_INACTIVE_LOCKED[ch%2];
					else color = TABLE_COLOR_BUTTON_INACTIVE[ch%2];
...
//				if (state == 0) color = (block->groupNr == 0) ? TABLE_COLOR_directIO_inactive : VAL_OFFWHITE;
			...
			default:
				...
				DIGITALBLOCKS_displayDigitalChannels_old (seq, block, ch);
				return;
		}
	}
...
//		startTime = clock ();
			...
			for (i = 0; i < seq->maxDigitalChannel; i++) {
				DIGITALBLOCKS_displayDigitalChannels_old (seq, b, i);
			}	
	//		duration = timeStop_s (startTime);
...
void DIGITALBLOCKS_displayAllAnalogChannels_fast (t_sequence *seq, t_digitalBlock **bl, int nBlocks, int panel, int ctrlAnalog)
{
	...
	SetTableCellRangeAttribute (panel, ctrlAnalog, VAL_TABLE_ENTIRE_RANGE, ATTR_CELL_DIMMED, 1);
	SetTableCellRangeAttribute (panel, ctrlAnalog, VAL_TABLE_ENTIRE_RANGE, ATTR_CELL_MODE, VAL_INDICATOR);
	...
	for (i = 0; i < N_DAC_CHANNELS; i++) {
		SetTableCellRangeAttribute (panel, ctrlAnalog,
									VAL_TABLE_ROW_RANGE(i+1), //MakeRect (i+1, 1, 1, nBlocks),
									ATTR_TEXT_BGCOLOR, TABLE_COLOR_BUTTON_INACTIVE_DAC[(i+1)%2]);
		//SetTableCellRangeAttribute (panel, ctrlAnalog,
		//					VAL_TABLE_ROW_RANGE(i+1), //MakeRect (i+1, 1, 1, nBlocks),
...
{
	int i, k, ch;
	int color, cellMode, dimmed, state;
	Point cell;
	char *txt;
	int first;
	char help[20];
	...
	for (i = startCh; i < stopCh; i++) {
		...
		switch (seq->DIO_channelStatus[i]) {
			...
			case DIO_CHANNELSTATUS_ALWAYSON:
				...
				//SetTableCellRangeAttribute (panel, ctrlDigital, channelCells,
				...
				for (k = 0; k < nBlocks; k++) {
					...
					if (strlen(txt) > 0) {
				    	SetTableCellAttribute (panel, ctrlDigital, cell, ATTR_CTRL_VAL, txt);							 
					}
				}
			...
			case DIO_CHANNELSTATUS_ALWAYSOFF:
				...
				color = TABLE_COLOR_channel_alwaysOff;
				SetTableCellRangeAttribute (panel, ctrlDigital,
											MakeRect (i+1, 1, 1, nBlocks),
											ATTR_CELL_MODE, VAL_INDICATOR);
				SetTableCellRangeAttribute (panel, ctrlDigital,
											MakeRect (i+1, 1, 1, nBlocks),
											ATTR_TEXT_COLOR, VAL_RED);
				for (k = 0; k < nBlocks; k++) {
					cell = MakePoint (k+1, i+1);	 
					switch (k  % 3) {
						 case 0: 
		...
		SetTableCellRangeAttribute (panel, ctrlDigital, channelCells, //MakeRect (i+1, 1, 1, nBlocks),
	...
	for (i = 0; i < nBlocks; i++) {
		if (DIGITALBLOCKS_isFirstOfCompressedGroup (bl[i])) {
			cells = MakeRect (1, i+1, seq->maxDigitalChannel, 1);
			...
			if (bl[i]->bitmapPtrArr == NULL) DIGITALBLOCKS_createBitmaps (seq, bl[i], -2, -2);
...
void DIGITALBLOCKS_displayAllInTable_fast (t_sequence *seq, int panel, int ctrlHeader, int ctrlDac, int ctrlDigital)
{
	int nBlocks;
	int i, ch;
	int color, cellMode, dimmed, state, width;
	t_digitalBlock **bl;
	Rect cells;
...
void DIGITALBLOCKS_EDIT_paste (t_sequence *seq, int position, int behind)
{
	...
	for (i = 1; i <= n; i++) {
		...
		if (insertPos > ListNumItems(seq->lDigitalBlocks)) insertPos = END_OF_LIST;
		...
		for (ch = 0; ch < N_DAC_CHANNELS; ch++) {
			...
			if (pastedBlock->waveforms[ch] > ListNumItems (blockClipboardWaveforms)) 
			...
			if (pastedBlock->waveforms[ch] > 0) {
				...
				pastedWfm->add = WFM_ID_UNCHANGED;; // DEGUB --> correctly paste added waveform
...
void DIGITALBLOCKS_insertTableColumns (int panel, int ctrlHeader, int ctrlDac, int ctrlDigital, int startCol, int nColumns)
{
	...
	cells = MakeRect (1, startCol, nRows, nColumns);
        ...
        case EVENT_LEFT_CLICK_UP:
			...
			if (cell.x == BLOCK_TABLE_COL_directIO) {
				...
				globalDigitalValues[channel] = !globalDigitalValues[channel];
	           	HARDWARE_sendDirectDigitalValue (channel, globalDigitalValues[channel]);
...
*/
	switch (event)
	{
		...
		case EVENT_LEFT_CLICK:
			...
			DIGITALBLOCKS_displayDigitalChannels_old (seq, p, ch); 
            setChanges (seq, 1);
			ProcessDrawEvents();
		...
		void *callbackData, int eventData1, int eventData2)
{
	int hScrollOffset;
	int ch;
	Point cell, newCell, cellBlockNr;
	t_sequence *seq;
	t_digitalBlock *p, *p2;
	double dTime;
...
//			}
	...
	switch (event)
	{
		case EVENT_KEYPRESS:
			...
			if (selection.left+selection.width-1 > ListNumItems (seq->lDigitalBlocks)) {
